...
const RequestBodySchema = z.object({
  count: z.number().openapi({
    example: 1,
    description: 'The number of times to decrement the feature usage.',
  }),
...
const route = createRoute({
  method: 'post',
  path: '/subscriptions/decrement-feature-used',
  summary: 'Decrement feature usage',
  description:
    'Decrements the usage count of a specific feature for the authenticated user.',
  tags: ['Subscription'],
  request: {
    body: {
      content: {
  ...
  responses: {
    200: {
      description: 'Feature usage decremented successfully',
    },
    400: {
...
const createHandler = (
  ...
  return async (c) => {
    try {
      ...
      await subscriptionAppService.decrementFeatureUsed(
        companyId,
        count,
        featureCode,
      );
      ...
      const response = new ApiSuccessResponse(
        StatusCode.OK,
        null,
        'Feature usage decremented successfully',
      );
